 ...
 *       cardsCorrect } — owned by app.js. (cardsAnswered/cardsCorrect drive
 *       the study-volume + accuracy badges.)
 *   - localStorage "anpiLearn"  : { level, done:{unitId:true} } — lesson
 *       completion, owned by app.js.
 *   - localStorage "kanaFlashcardResults" : [ {deck,set,correct,total,pct,
 *       [type:"game"]} ] — saved deck-session history, owned by app.js.
 *       Deck study sessions have NO `type`; game sessions set type:"game".
 *       Used for the deck-mastery badges (best pct reached per deck).
...
(function () {
  ...
  var RESULTS_KEY = "kanaFlashcardResults"; // read-only (owned by app.js)
  ...
  function readResults() {
    try { var r = JSON.parse(localStorage.getItem(RESULTS_KEY)); return Array.isArray(r) ? r : []; } catch (e) { return []; }
  }
  ...
  function buildContext() {
    ...
    // Flashcard study volume + accuracy (cumulative, from anpiStats).
    var cardsAnswered = stats.cardsAnswered || 0;
    var cardsCorrect = stats.cardsCorrect || 0;
    var accuracy = cardsAnswered > 0 ? (cardsCorrect / cardsAnswered) : 0;

    // Deck mastery — from the saved per-session history. A deck counts as
    // "mastered" once any STUDY session for it scored ≥90% (game sessions,
    // tagged type:"game", are excluded). `deck` is the deckKey; `pct` is the
    // session's percent (we recompute from correct/total if it's missing).
    var bestPctByDeck = {};
    var deckPerfect = false;
    readResults().forEach(function (r) {
      if (!r || r.type === "game") return;
      var total = Number(r.total);
      if (!r.deck || !(total > 0)) return;
      var pct = Number(r.pct);
      if (!isFinite(pct)) pct = Math.round((Number(r.correct) || 0) / total * 100);
      if (pct >= 100) deckPerfect = true;
      if (!(r.deck in bestPctByDeck) || pct > bestPctByDeck[r.deck]) bestPctByDeck[r.deck] = pct;
    });
    var decksMastered = 0;
    for (var dk in bestPctByDeck) {
      if (Object.prototype.hasOwnProperty.call(bestPctByDeck, dk) && bestPctByDeck[dk] >= 90) decksMastered++;
    }
  ...
  var BADGES = [
    ...
    { id: "streak7",   cat: "streak", icon: "⚔️",  name: "Week Warrior",  desc: "7-day streak",   test: function (c) { return c.streak >= 7; } },
    ...
    { id: "streak100", cat: "streak", icon: "🎖️",  name: "Centurion",     desc: "100-day streak", test: function (c) { return c.streak >= 100; } },
    ...
    /* Flashcards — study volume + accuracy + deck mastery (real tracked data) */
    { id: "cards100",  cat: "cards", icon: "🃏", name: "Getting Going", desc: "Answer 100 flashcards",   test: function (c) { return c.cardsAnswered >= 100; } },
    { id: "cards500",  cat: "cards", icon: "📇", name: "Card Carrier",  desc: "Answer 500 flashcards",   test: function (c) { return c.cardsAnswered >= 500; } },
    { id: "cards2k",   cat: "cards", icon: "🗂️", name: "Card Scholar",  desc: "Answer 2,000 flashcards", test: function (c) { return c.cardsAnswered >= 2000; } },
    { id: "cards10k",  cat: "cards", icon: "🀄", name: "Card Sensei",   desc: "Answer 10,000 flashcards", test: function (c) { return c.cardsAnswered >= 10000; } },
    { id: "acc90",     cat: "cards", icon: "🎯", name: "Sharpshooter",  desc: "90%+ accuracy over 100+ cards", test: function (c) { return c.cardsAnswered >= 100 && c.accuracy >= 0.9; } },
    { id: "deck-perfect", cat: "cards", icon: "💯", name: "Flawless Run", desc: "Finish a deck with 100% correct", test: function (c) { return c.deckPerfect; } },
    { id: "decks3",    cat: "cards", icon: "🎴", name: "Deck Tamer",    desc: "Master 3 decks (90%+ in a session)", test: function (c) { return c.decksMastered >= 3; } },
    { id: "decks10",   cat: "cards", icon: "🏵️", name: "Deck Master",   desc: "Master 10 decks (90%+ in a session)", test: function (c) { return c.decksMastered >= 10; } },
  ...
  function inputKey() {
    ...
    try { c = localStorage.getItem(RESULTS_KEY); } catch (e) { c = null; }
    return (a || "") + " " + (b || "") + " " + (c || "");
  }
  ...
  // A badge's details show in ONE shared card, fixed and centered near the top
  // of the viewport. It's attached to <body> (not inside the wall), so the wall's
  // overflow can't clip it — borders and text are always fully visible. It just
  // swaps its text on hover/focus; no movement, no per-badge edge math.
  function ensureDetail() {
    var d = document.getElementById("bwDetail");
    if (!d) {
      ...
      d.innerHTML =
        '<span class="bwd-name"></span>' +
        '<span class="bwd-desc"></span>' +
        '<span class="bwd-status"></span>';
  ...
  function renderInto(el) {
    ...
    CATS.forEach(function (cat) {
      ...
      inCat.forEach(function (b) {
        ...
        var status = got ? ("✓ Earned" + (when ? " · " + when : "")) : "🔒 Locked";
        html +=
          '<div class="bw-badge ' + (got ? "is-earned" : "is-locked") + '" role="listitem" tabindex="0"' +
            ' aria-label="' + esc(aria) + '" data-name="' + esc(b.name) + '" data-desc="' + esc(b.desc) +
            '" data-status="' + esc(status) + '" data-got="' + (got ? "1" : "0") + '">' +
    ...
    // Wire the shared detail card once (delegated; survives re-renders since the
    // listeners live on the persistent wall element, not the rebuilt children).
    if (!el._tipWired) {
      el._tipWired = true;
      var detail = ensureDetail();
      var fill = function (b) {
        if (!b) return;
        detail.querySelector(".bwd-name").textContent = b.getAttribute("data-name") || "";
        detail.querySelector(".bwd-desc").textContent = b.getAttribute("data-desc") || "";
        detail.querySelector(".bwd-status").textContent = b.getAttribute("data-status") || "";
        detail.classList.toggle("is-earned", b.getAttribute("data-got") === "1");
        detail.classList.add("show");                 // show first so we can measure it
        // Park the card right by its badge, then pull it inward so the whole box
        // (borders included) always stays on screen. Instant — no transition.
        var m = 8;
        var br = b.getBoundingClientRect();
        var cr = detail.getBoundingClientRect();
        var vw = window.innerWidth || document.documentElement.clientWidth;
        var vh = window.innerHeight || document.documentElement.clientHeight;
        var left = br.left + br.width / 2 - cr.width / 2;       // centered over the badge…
        if (left < m) left = m;                                 // …then clamped into view
        if (left + cr.width > vw - m) left = vw - m - cr.width;
        var top = br.top - cr.height - 10;                      // above the badge
        if (top < m) top = br.bottom + 10;                      // no room above → below it
        if (top + cr.height > vh - m) top = Math.max(m, vh - m - cr.height);
        detail.style.left = Math.round(left) + "px";
        detail.style.top = Math.round(top) + "px";
      };
      var hide = function () { detail.classList.remove("show"); };
      el.addEventListener("mouseover", function (e) { var b = e.target.closest && e.target.closest(".bw-badge"); if (b) fill(b); });
      el.addEventListener("mouseleave", hide);
      el.addEventListener("focusin", function (e) { var b = e.target.closest && e.target.closest(".bw-badge"); if (b) fill(b); });
