...
(function () {
  ...
  function bindUI() {
    ...
    // Click the dark backdrop (outside the modal box) to close — ignored for 400ms after opening so
    // the click that opened it (and any touch ghost-click) can't immediately dismiss it.
    var overlay = $("authOverlay");
    if (overlay) overlay.addEventListener("click", function (e) {
      if (e.target === overlay && Date.now() - modalOpenedAt > 400) closeModal();
    });
    // Escape also closes.
