...
(function () {
  ...
  async function pushHighScore(deckKey, mode, value, streak) {
    if (!ready()) return;
    var streakN = Math.max(0, parseInt(streak, 10) || 0);
    try {
      ...
      if (row) {
        if (toBig(value) > toBig(row.score)) {
          var u = await sb.from("high_scores")
            .update({ score: String(value), best_streak: streakN, achieved_at: new Date().toISOString() })
            .eq("id", row.id);
          if (u.error) throw u.error;
        }
      } else {
        var ins = await sb.from("high_scores").insert({
          user_id: user.id, deck_id: deckKey, difficulty: mode, score: String(value), best_streak: streakN
        });
  ...
  async function doSignUp() {
    ...
    try {
      var r = await sb.auth.signUp({
        email: email,
        password: pass,
        options: { emailRedirectTo: "https://anpi.pages.dev" }
      });
