...
(function () {
  ...
  var LEARN_KEY = "anpiLearn";                // matches app.js LEARN_KEY (guided-path state)
  ...
  var KanaSync = {
    ...
    // Guided-path: called by app.js saveLearnState() after every level pick /
    // lesson completion. Fire-and-forget upsert of the whole learn state.
    onLearnChange: function () {
      if (!ready()) return;
      pushLearnState().catch(noop);
    }
  };
  ...
  // ---- Guided-path (Learn) state: level + completed lessons --------------------
  function readLearnLocal() {
    try { var s = JSON.parse(localStorage.getItem(LEARN_KEY) || "{}"); return s && typeof s === "object" ? s : {}; }
    catch (e) { return {}; }
  }
  function truthyKeys(obj) {
    var out = {}; if (obj && typeof obj === "object") Object.keys(obj).forEach(function (k) { if (obj[k]) out[k] = true; });
    return out;
  }
  // Push the whole learn state up (upsert one row per user).
  async function pushLearnState() {
    if (!ready()) return;
    var st = readLearnLocal();
    try {
      var up = await sb.from("learn_state").upsert({
        user_id: user.id,
        level: st.level ? clampStr(st.level, 32) : null,
        done: truthyKeys(st.done),
        updated_at: new Date().toISOString()
      }, { onConflict: "user_id" });
      if (up.error) throw up.error;
    } catch (e) { /* learn_state table not set up yet — silent, local still works */ }
  }
  // Merge cloud learn state into local on sign-in: union completed lessons; the
  // more-advanced side's level wins (so a fresh device's default doesn't clobber
  // an established account). Returns true if a cloud row existed.
  async function hydrateLearnState() {
    if (!ready()) return false;
    try {
      var res = await sb.from("learn_state").select("level,done").eq("user_id", user.id).limit(1);
      if (res.error) throw res.error;
      var cloud = (res.data && res.data[0]) || null;
      var local = readLearnLocal();
      var localDone = truthyKeys(local.done);
      var cloudDone = truthyKeys(cloud && cloud.done);
      var mergedDone = {};
      Object.keys(cloudDone).forEach(function (k) { mergedDone[k] = true; });
      Object.keys(localDone).forEach(function (k) { mergedDone[k] = true; });
      var lc = Object.keys(localDone).length, cc = Object.keys(cloudDone).length;
      var mergedLevel = (cc >= lc ? (cloud && cloud.level) : local.level) || local.level || (cloud && cloud.level) || null;
      try { localStorage.setItem(LEARN_KEY, JSON.stringify({ level: mergedLevel, done: mergedDone })); } catch (e) {}
      return !!cloud;
  ...
  async function onSignedIn(u, offerMigrate) {
    ...
    await hydrateLearnState();          // pull guided-path level + completed lessons
    pushLearnState().catch(noop);       // converge the cloud with the merged result
    try { window.dispatchEvent(new Event("anpi-learn-synced")); } catch (e) {}
    renderAuthState();
    refreshBadges();
    if (pendingMigration && (pendingMigration.reports.length || pendingMigration.highs.length)) {
      ...
      // Auto-close ONLY after a deliberate sign-in (offerMigrate). On page-load auto-auth
      // (getSession on every load) the modal isn't open, and arming this timer would later
      // close the modal if the user opened it themselves in the meantime — which made the
      // account/profile panel "close itself on the first open" for signed-in users.
      if (offerMigrate) setTimeout(closeModal, 1100);
    }
  }
  ...
  var modalOpenedAt = 0;
  function openModal() { var o = $("authOverlay"); if (o) o.classList.add("show"); modalOpenedAt = Date.now(); setMsg(""); refreshBadges(); }
  ...
  function bindUI() {
    ...
    // NOTE: intentionally NO "click outside / on the backdrop to close" handler — it was dismissing
    // the account panel unexpectedly. The modal closes only via the explicit "Close" button
    // (#authClose) or Sign Out. (modalOpenedAt is kept for reference but no longer gates a close.)
    var pass = $("authPass");
