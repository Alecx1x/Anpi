...
(function () {
  ...
  var KanaSync = {
    ...
    onHighScore: function (deckKey, mode, value) {
      if (!ready()) return;
      pushHighScore(deckKey, mode, String(value)).catch(noop);
    },
  ...
  // Cloud writers
  // ==========================================================================
  async function pushHighScore(deckKey, mode, value) {
    if (!ready()) return;
    try {
      var res = await sb.from("high_scores")
        .select("id,score")
        .eq("user_id", user.id).eq("deck_id", deckKey).eq("difficulty", mode)
        .limit(1);
      var row = res.data && res.data[0];
      if (row) {
        if (toBig(value) > toBig(row.score)) {
          await sb.from("high_scores")
            .update({ score: String(value), achieved_at: new Date().toISOString() })
            .eq("id", row.id);
        }
      } else {
        await sb.from("high_scores").insert({
          user_id: user.id, deck_id: deckKey, difficulty: mode, score: String(value)
        });
      }
    } catch (e) { /* silent */ }
  }
  ...
  async function pushReport(record) {
    ...
    try {
      await sb.from("reports").insert({
        user_id: user.id,
        deck_id: String(record.deck || ""),
        total_cards: record.total || 0,
        correct_count: record.correct || 0,
        incorrect_count: Math.max(0, (record.total || 0) - (record.correct || 0)),
        report_data: record
      });
    } catch (e) { /* silent */ }
  }
    ...
    try {
      ...
      cards.forEach(function (c) {
        ...
        if (row) {
          ...
          inserts.push({
            user_id: user.id, deck_id: deckId, card_id: String(c.id),
  ...
  async function fetchProfileName() {
    if (!ready()) return;
    try {
      var r = await sb.from("profiles").select("username").eq("id", user.id).limit(1);
      if (r.data && r.data[0] && r.data[0].username) {
        profileName = r.data[0].username;
        renderAuthState();
      }
    } catch (e) {}
  }
  ...
  async function doSignUp() {
    ...
    try {
      var r = await sb.auth.signUp({
        email: email,
        password: pass,
        options: { emailRedirectTo: "https://benevolent-mooncake-f01661.netlify.app" }
      });
      if (r.error) { setMsg(r.error.message, true); return; }
  ...
  function openModal() { var o = $("authOverlay"); if (o) o.classList.add("show"); setMsg(""); }
  ...
  // ==========================================================================
  function wrapAppFns() {
    if (typeof window.setHigh === "function" && !window.setHigh.__synced) {
      var _setHigh = window.setHigh;
      window.setHigh = function (deckKey, mode, v) {
        var out = _setHigh.apply(this, arguments);
        try { KanaSync.onHighScore(deckKey, mode, (typeof v === "bigint" ? v.toString() : String(v))); } catch (e) {}
        return out;
      };
      window.setHigh.__synced = true;
    }
